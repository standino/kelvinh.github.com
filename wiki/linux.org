...
* [[http://w3.ibm.com/connections/wikis/home/wiki/Wbd0c3129b6b7_4e5f_ba7d_2bb397faa12f/page/Setup%20Ubuntu%20as%20your%20primary%20workstation?lang=zh][Setup Ubuntu as your primary workstation]]

[[http://w3.tap.ibm.com/w3ki03/display/linuxportal/Ubuntu+Linux+Client+Acceleration+Guide][Ubuntu Linux Client Acceleration Guide]]

[[http://w3.tap.ibm.com/w3ki03/display/linuxportal/Install+PAE+Kernel+for+Machines+with+more+than+3+GB+Memory][Install PAE Kernel for Machines with more than 3 GB Memory]]
...
initrd /initrd.gz
 ...
 - download ocdc-repository_1.5_all.deb  from http://ocdc.hursley.ibm.com/ocdc/pool/IBM/o/ocdc-repository/ 
...
** WID7 and WAS 7
 ...
 - start IBMIM and set the repository to  http://btsjazz.dyn.webahead.ibm.com:8000. Please check this for more detail:  [[Notes://CSDLD01/48256F8C0020B1A3/BB2207E44A8BD1BC052566AB007D9C67/CF81F260E602C4F348257777002C5152 ]]
...
** [[http://ciscovpn.watson.ibm.com/instructions.php][Emerging VPN Technologies Pilot]]

  
  rcxasa.watson.ibm.com
  
  sudo yum install anyconnect ibm-config-anyconnect


    USER ID: changwei@cn.ibm.com
    PASSWORD: nNvinW)8qUa0 Case Sensitive
 

Category: Linux, Ubuntu Tags: Ubuntu, Ubuntu 8.10, VPN, VPNC
Easier VPN Setup with Ubuntu 8.10
Posted November 6th, 2008 by nazham
...
https://ltc3.linux.ibm.com/wiki/LinuxDesktop
...
https://ltc3.linux.ibm.com/wiki/LTCDesktop/LUDContrib
...
[[https://w3.tap.ibm.com/w3ki03/display/linuxportal/IBM+LEAP+Wifi+Wireless+Authentication#IBMLEAPWifiWirelessAuthentication-IBMLEAPWifiWirelessAuthenticationsteps][IBM LEAP upgrade]]

https://w3.tap.ibm.com/w3ki/display/RSTL/WiFi-nonWin

 - install the driver according to [[../etc/FirstTimeHowTo.html][First time how to]]
...
wpa-eap LEAP
wpa-ap-scan 2
wpa-identity changwei@cn.ibm.com
wpa-password nNvinW)8qUa0


  </src>
 password for ring: institute4t60

 - execute *sudo myleap up|down|bounce* to start|stop|restart wireless.
 - 以后使用 sudo pon dsl-provider 拨号上网，使用sudo poff dsl-provider
   下线。sudo plog查看拨号日志。 

* AT&T Global Network Client

Web site to get a ID for AGN: http://w3-03.ibm.com/tools/it/ittools.nsf/main/ATTNC_new_users
<src>
# agn
# Tested in GUTSY/HARDY
sudo apt-get install agnclient
</src>
...
* Wireless Misc
 ...
 1. install the driver according to [[../etc/FirstTimeHowTo.html][First time how to]]
 2. 
 2. Click Network Manager (as pictured below) and select Connect to Other Wireless Network
 3. Network name is always IBM, security is LEAP. Key manager WPA-EPA
 4. Fill in your [[../other/index.html#sec8][username and password]] from the mail you were sent:
   changwei@cn.ibm.com nNvinW)8qUa0



[[../etc/connecting_moinmoin_wastes_my_time_by_not_handling_images_with_spaces.png]]

[[../etc/Connected.png]]


execute *sudo myleap up* to start wireless.

Install wireless drivers according to
http://madwifi.org/wiki/UserDocs/Distro/Debian/MadWifi

http://forum.ubuntu.org.cn/viewtopic.php?p=735651&sid=92cb9ae9f464e795781824204190ddf3

https://ltc3.linux.ibm.com/wiki/WirelessConnectivityOnLinux

https://w3.tap.ibm.com/w3ki/display/UHH/Installing+Hardy+on+a+T61p



exe *echo "ath_pci" | sudo tee -a /etc/modules*

 start wireless drivers : modprobe ath_pci

 start LED: sudo sysctl -w dev.wifi0.softled=1
...
sudo ln -s /usr/lib/firefox/plugins/lud-gpws.so /usr/lib/xulrunner-addons/plugins/ibmgpws.so
</src>

* AT&T Global Network Client Misc
...
I found some problem with the agnLogArchive.tcl, after you reboot the machine execute:
<src>
# kill the agnLogArchive.tcl script
killall agnclient
...
How to get the latest version of agn. This is not required, it's just here for reference:
<src>
# more up to date agn client
mkdir tmp/agn ; cd tmp/agn
...
sudo dpkg -i agnclient-1.0-2.0.1.3000.i386.deb
# just to make sure all dependencies are installed
sudo apt-get install
</src>

  at&t linux website is: http://www.attnetclient.com/linux/ * Forum about ubuntu: http://attnetclient.com/forum/viewtopic.php?p=2309#2309 * How to create deb from the rpm official release: http://blog.afmacedo.com/?p=11

* Repository 

~/bin/sources.list:

< include file="~/ideas/bin/sources.list" markup="src" lang="sh">
...
You can get the debs here: ftp://pokgsa.ibm.com/projects/o/openclient/lotus/notes/beta/8.5/latest/

Download all of them and then install ibm_lotus_notes-8.5-20080819.0130.i586.deb first. This will satisfy all dependencies for the
remaining files, which you can install either one-at-a-time or as a block. 

Please note that this is not a software source that you can define in Synaptic. If there's an update to these files you will have
to re-download them and update them manually. 
...
* [[https://ltc3.linux.ibm.com/wiki/LinuxDesktop/Fetchnotes][fetch notes]]

http://ibmforums.ibm.com/forums/thread.jspa?messageID=1315054&tstart=0&#1315054

http://ibmforums.ibm.com/forums/thread.jspa?messageID=1218790&tstart=0&#1218790

https://ltc3.linux.ibm.com/wiki/LinuxDesktop/Fetchnotes
...
http://kbase.redhat.com/faq/docs/DOC-13668
...
< include file="~/ideas/bin/myinstall" markup="src" lang="sh">


* IPv6

<  include file="/home/will/ideas/Muse/tools/ipaddress.txt" markup="src" >
...
ssh -l changwei etlbdswws01.etl.ibm.com  "wc -l /web/quotelogtool/src/log4j.properties"

ssh  changwei@etlbdswws01.etl.ibm.com  "wc -l /web/quotelogtool/src/log4j.properties"
...
https://w3-connections.ibm.com/wikis/home?lang=en_US#/wiki/W693fcc3e568b_4f4f_9b2c_30a585ebb3d3/page/%28optional%29%20Add%20additional%20repositories
...
sudo yum grouplist|grep IBM

sudo yum groupinstall "IBM Client Java" "IBM Lotus Notes" "IBM Sametime" 
...
http://ibmforums.ibm.com/forums/thread.jspa?messageID=1921274&tstart=0&#1921274
...
http://www.crsay.com/wiki/wiki.php/server/proftp/install-virtualuser-manage
...
disable firewalld to allow isam to upload the scan resluts. 

先关闭firewalld, 在isam 里面启动scan,然后看Ｌog，如果看到启动wst, 立刻启动firewalld. 

sudo systemctl stop firewalld.service  

* [[https://w3-connections.ibm.com/wikis/home?lang=zh_CN#!/wiki/W693fcc3e568b_4f4f_9b2c_30a585ebb3d3/page/Installed%20IBM%20Notes%209%20-%20SUT%20doesn't%20work][Installed IBM Notes 9 - SUT doesn't work]]
...
[root@bxv5v1054 network-scripts]# cat ifcfg-Auto_eth1 
TYPE=Ethernet
BOOTPROTO=none
IPADDR=9.110.82.113
PREFIX=24
...
IPV6INIT=no
NAME="Auto eth1"
UUID=0a231697-295f-4eda-b4c4-ca7577bef0b9
ONBOOT=yes
NETMASK=255.255.255.0
DEVICE=Auto_eth1
USERCTL=no
HWADDR=00:50:56:AA:53:75
LAST_CONNECT=1369120609
DNS1=9.181.2.101
DNS2=9.181.2.102
